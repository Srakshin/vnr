...
- **`backend/`**: Contains the Python FastAPI application.
  ...
  - `services/routing_service.py`: Standard OSRM routing plus smart-routing graph logic that excludes blocked roads and penalizes risky roads.
...
- **Smart route endpoint**: `POST /api/route/smart` with status-tagged roads and `[lon, lat]` start/end points.
...
- Keep smart route data isolated from the standard route overlay using dedicated Zustand state such as `smartRouteGeoJSON`, metadata, and warnings.
...
- For smart routing, verify blocked roads are excluded, risky roads are used only when necessary, start/end points snap to non-blocked roads, and the UI warning appears when only risky routes are available.
